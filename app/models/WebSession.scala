...
class WebSession(remoteIP: String) extends Actor {
  ...
  def checkProgLangSolution(gentry: GrammarEntry, studentGrammar: BNFGrammar)(implicit opctx: GlobalContext): OpRes = {
    ...
    val startSZ = eqctx.startSize
    val maxSZ = eqctx.maxSize
    val nos = eqctx.nOfTestcases

    def checkGSubsetRef(refParser: AntlrParser) = {
      try {
        var stop = false
        val task = Util.scheduleTask(() => { stop = true }, eqctx.timeOut)
        //generators for g          
        val gGen = new SizeBasedRandomAccessGenerator(g.fromCNF, eqctx.maxSize)
        val gEnums = (startSZ to maxSZ).map { sz => gGen.getSamplingEnumerator(g.fromCNF.start, sz, nos) }.toArray
        var toExploreSizes = (startSZ to maxSZ).toList
        val rand = new java.util.Random()
        var break = false
        var ctrex: Option[Word] = None
    ...
    if (g.cnfGrammar.rules.isEmpty) {
      ...
      Partial("Checking if your grammar accepts all valid string generated by the initial grammar..." +
        ...
        Future {
          ...
          var stop = false
          val task = Util.scheduleTask(() => { stop = true }, eqctx.timeOut)
          val initEnums = (startSZ to maxSZ).map { sz => initGen.getSamplingEnumerator(init.fromCNF.start, sz, nos) }.toArray
            ...
            result match {
              ...
              case None =>
                ...
                Partial("Your grammar accepts all valid strings generated by initial grammar!" +
                  ...
                  Future { checkGSubsetRef(refParser) })
            }
          }
        })
    }
