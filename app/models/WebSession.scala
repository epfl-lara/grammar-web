...
class WebSession(remoteIP: String) extends Actor {
  ...
  def checkProgLangSolution(gentry: GrammarEntry, studentGrammar: BNFGrammar)(implicit opctx: GlobalContext): OpRes = {
    ...
    if (g.cnfGrammar.rules.isEmpty) {
      ...
      def cleanup() {
        //cleanup        
        (new java.io.File(bindir)).listFiles().filter(_.exists()).foreach { fl =>
          try { fl.delete() }
          catch { case e: FileNotFoundException => ; }
        }
      }
      Partial("Checking if your grammar accepts all valid string generated by the initial grammar..." +
        "\n(This might take 1min)",
        Future {
          try {
            //parsers and generators for ref
            val refGen = new SizeBasedRandomAccessGenerator(ref.fromCNF, eqctx.maxSize)
            val refParser = new AntlrParser(ref)
            //parsers and generators for g
            val gEG = g.fromCNF
            val gGen = new SizeBasedRandomAccessGenerator(gEG, eqctx.maxSize)
            val gParser = if (GNFUtilities.hasIndirectLeftRecursion(g))
              new CYKParser(g.cnfGrammar)
            else
              new AntlrParser(g)
            //generators for init
            val initGen = new SizeBasedRandomAccessGenerator(init.fromCNF, eqctx.maxSize)

            //for every size from startSize to 'maxWordSize' check if every word 
            //in g2 is contained in refg
            val startSize = eqctx.startSize
            val maxSize = eqctx.maxSize
            val nos = eqctx.nOfTestcases

            var result: Option[OpRes] = None
            //create a thread that sets stop flag in 'timeout' ms
            var stop = false
            val task = Util.scheduleTask(() => { stop = true }, eqctx.timeOut)
            val initEnums = (startSize to maxSize).map { sz => initGen.getSamplingEnumerator(init.fromCNF.start, sz, nos) }.toArray

            var toExploreSizes = (startSize to maxSize).toList
            val rand = new java.util.Random()
            var break = false //initial value of break is g == init
            while (!break && !toExploreSizes.isEmpty && !stop && !opctx.abort) {
              val exploreIndex = rand.nextInt(toExploreSizes.size) //randomly sample a size
              val size = toExploreSizes(exploreIndex)
              val initEnum = initEnums(size - 1)
              if (initEnum.hasNext) {
                val y = initEnum.next
                if (refParser.parse(y) && !gParser.parse(y)) {
                  //parsed by 'ref' by not by 'g'	         	          
                  /*val msg = "Found counter-example of size " + size + " : " + y
	          	  println(msg)*/
                  result = Some(Last("Your grammar does not accept the valid string: " +
                    wordToString(y) + " generated by the initial grammar"))
                  break = true;
                }
              } else
            ...
            if (opctx.abort) {
              ...
              result match {
                case Some(fedb) =>
                  ...
                  fedb
                case None =>
                  //check if 'g' is contained in 'ref'
                  Partial("Your grammar accepts all valid strings generated by initial grammar!" +
                    " Checking if it generates invalid strings ...",
                    Future {
                      try {
                        val res = new SamplingBasedEquivalenceChecker(ref).isSubset(g)
                        res match {
                          case List() =>
